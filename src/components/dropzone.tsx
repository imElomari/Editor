...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  const { maxFiles, maxFileSize, inputRef, isSuccess, files, isDragActive, isDragReject } =
    useDropzoneContext()
  ...
  if (isDragActive && !isDragReject) {
    return (
      <div className={cn('flex flex-col items-center gap-y-4 py-8', className)}>
        ...
        <p className="text-primary font-medium animate-pulse">{t('common:dropzone.dropFile')}</p>
      </div>
    )
  }
