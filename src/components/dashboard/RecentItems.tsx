...
export default function RecentItems({
  ...
  const formatRelativeTime = (dateString: string) => {
    ...
    if (diffInSeconds < 3600)
      return `${Math.floor(diffInSeconds / 60)} ${t('dashboard:time.minutesAgo')}`
      ...
      <TabsContent value="labels" className="m-0 space-y-4">
        <Card className="overflow-hidden border-t-4 border-t-primary/50">
          ...
          >
            <div>
              <CardTitle
                className={cn('text-xl flex items-center gap-2', isRTL && 'flex-row-reverse')}
              >
                        ...
                        <div className="min-w-0">
                          ...
                          >
                            <span className={cn('flex items-center', isRTL && 'flex-row-reverse')}>
                              <Icons.clock
                                className={cn('h-3 w-3 flex-shrink-0', isRTL ? 'ms-1' : 'me-1')}
                              />
