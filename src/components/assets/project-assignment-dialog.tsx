    ...
    <ModalWrapper isOpen={isOpen} onClose={onClose}>
      <div className="space-y-4">
        <div className="space-y-2">
          <h2 className="text-xl font-semibold">{t('assets:card.assignProject.title')}</h2>
          <p className="text-muted-foreground">
            {t('assets:card.assignProject.placeholdre', asset?.name)}
          </p>
        </div>
        <div className="space-y-2">
          <ProjectSelector
        ...
        <div className="flex justify-end space-x-2">
          ...
          <Button onClick={onAssign} disabled={isLoading || !selectedProjectId}>
            {isLoading
              ? t('assets:card.assignProject.assigning')
              : t('assets:card.assignProject.confirm')}
          </Button>
        </div>
      </div>
    </ModalWrapper>
